...
namespace Covenant.Services
{
    public class RequestService : IRequestService
    {
        ...
        public async Task<Result<Guid>> CompanyCreateRequest(RequestCreateModel model)
        {
            ...
            var companyProfile = await companyRepository.GetCompanyProfileByCompanyId(companyId);
        ...
        public async Task<Result> FinalizeRequest(Guid requestId, RequestFinalizationDetailModel model)
        {
            if (model is null) return Result.Fail();
            Request request = await requestRepository.GetRequest(requestId);
            if (request is null) return Result.Fail(ApiResources.RequestNotAvailable);
            DateTime now = timeService.GetCurrentDateTime();
            Result rFinalize = request.Finalize(now);
            if (!rFinalize) return rFinalize;
            var finalizedBy = identityServerService.GetNickname();
            var entity = new RequestFinalizationDetail(request.Id, finalizedBy, model.Detail, now);
            var noteValue = "Order finalized";
            if (!string.IsNullOrEmpty(model.Detail)) noteValue += $" - {model.Detail}";
            var note = new RequestNote(request.Id, CovenantNote.Create(noteValue, CovenantNote.RedColor, entity.FinalizedBy).Value);
            var requestFinalizationDetail = await requestRepository.GetRequestFinalizationDetail(requestId);
            if (requestFinalizationDetail != null)
            {
                requestRepository.Delete(requestFinalizationDetail);
        ...
        public async Task<Result> IncreaseWorkersQuantityByOne(Guid requestId, string comment = null)
        {
            var request = await requestRepository.GetRequest(requestId);
            if (request is null) return Result.Fail(ApiResources.RequestNotAvailable);
            Result result = request.IncreaseWorkersQuantityByOne();
            if (!result) return result;
            await requestRepository.Update(request);
            await requestRepository.SaveChangesAsync();
            var data = await notificationDataRepository.GetAgencyData(requestId, NotificationType.NewWorkerRequested.Id);
            if (data != null && data.EmailNotification)
            {
                string message = await razorViewToStringRenderer.RenderViewToStringAsync("/Views/Notifications/OnNewWorkerRequested/AgencyTemplate.cshtml",
                ...
                await emailService.SendEmail(new EmailParams(data.AgencyEmail, "A new worker was requested", message));
            }
            return Result.Ok();
        }
        ...
        private async Task<Result<Request>> MapRequest(RequestCreateModel model, Guid companyId)
        {
            ...
            var rRequest = Request.AgencyCreateRequest(model.AgencyId, companyId, model.LocationId.Value, model.StartAt,
                model.JobPositionRateId, finishAt: model.FinishAt, jobIsOnBranchOffice: model.JobIsOnBranchOffice,
                workersQuantity: model.WorkersQuantity, breakIsPaid: model.BreakIsPaid,
                durationTerm: model.DurationTerm, employmentType: model.EmploymentType, isPunchCardOptionEnabled: model.PunchCardOptionEnabled);
