...
/// </summary>
public abstract class TaxTablesLoader(IDeductionsRepository repository, ILogger<TaxTablesLoader> logger, TaxType taxType)
{
	public Task LoadWeeklyTablesFromExcel(string excelPath, int year) =>
		LoadTablesFromExcel(excelPath, year, PayPeriod.Weekly);

	public Task LoadBiWeeklyTablesFromExcel(string excelPath, int year) =>
		LoadTablesFromExcel(excelPath, year, PayPeriod.BiWeekly);

	public Task LoadSemiMonthlyTablesFromExcel(string excelPath, int year) =>
		LoadTablesFromExcel(excelPath, year, PayPeriod.SemiMonthly);

	public Task LoadMonthlyTablesFromExcel(string excelPath, int year) =>
		LoadTablesFromExcel(excelPath, year, PayPeriod.Monthly);

	private async Task LoadTablesFromExcel(string excelPath, int year, PayPeriod payPeriod)
	{
		var changed = false;
		await ReadWorkbook(excelPath, year, payPeriod, async tax =>
		{
			await repository.CreateTax(tax);
			changed = true;
		});
		if (!changed) return;
		await repository.DeleteTax(year, payPeriod, taxType);
		await repository.SaveChangesAsync();
	}

	private async Task ReadWorkbook(string excelPath, int year, PayPeriod payPeriod, Func<TaxDeduction, Task> onReadValidRecord)
	{
		if (!File.Exists(excelPath)) return;
		using (var workbook = new XLWorkbook(excelPath))
		{
			IXLWorksheet federal = workbook.Worksheets.First();
			await ReadSheet(federal, year, payPeriod, onReadValidRecord);
		}
	}

	private async Task ReadSheet(IXLWorksheet worksheet, int year, PayPeriod payPeriod, Func<TaxDeduction, Task> onReadValidRecord)
	{
		IXLRow row = worksheet.FirstRowUsed();
		IXLRangeRow rangeRow = row.RowUsed();
		while (rangeRow != null && !rangeRow.IsEmpty())
		{
			TaxDeduction tax = GetRowValues(rangeRow, year, payPeriod);
			if (tax != null) await onReadValidRecord(tax);
			rangeRow = rangeRow.RowBelow();
		}
	}

	private TaxDeduction GetRowValues(IXLRangeRow rangeRow, int year, PayPeriod payPeriod)
	{
		(decimal? fromResult, decimal? toResult, string stringValue) = GetFromToValue(rangeRow, 1);
		if (!string.IsNullOrEmpty(stringValue))
		{
			logger?.LogError("Invalid tax value {Value}", stringValue);
			return null;
		}
		...
		return new TaxDeduction(fromResult.Value, toResult.Value, cc0, cc1, cc2, cc3, cc4, cc5, cc6, cc7, cc8, cc9, cc10,
			year, payPeriod, taxType);
	}
