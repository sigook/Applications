            ...
            migrationBuilder.Sql($"""
                ...
                BEGIN
                    IF EXISTS (SELECT 1 FROM "Request" WHERE "AgencyId" = '{AnabelleAgencyId}')
                        ...
                        OR EXISTS (SELECT 1 FROM "Candidates" WHERE "AgencyId" = '{AnabelleAgencyId}')
                        OR EXISTS (SELECT 1 FROM "Runners" WHERE "AgencyId" = '{AnabelleAgencyId}')
                    ...
                    IF EXISTS (SELECT 1 FROM "CompanyUser" WHERE "UserId" IN ({InvalidUsers}))
                        OR EXISTS (SELECT 1 FROM "WorkerDispatch" WHERE "CreatedBy" IN ({InvalidUsers}))
                        OR EXISTS (SELECT 1 FROM "WorkerProfile" WHERE "WorkerId" IN ({InvalidUsers}))
                        OR EXISTS (SELECT 1 FROM "AgencyPersonnel" WHERE "UserId" IN ({InvalidUsers}) AND "AgencyId" <> '{AnabelleAgencyId}')
                        OR EXISTS (
                            SELECT 1
                            FROM "RequestComissions" rc
                            JOIN "AgencyPersonnel" ap ON ap."Id" = rc."AgencyPersonnelId"
                            WHERE ap."UserId" IN ({InvalidUsers}))
                        OR EXISTS (
                            SELECT 1
                            FROM "RequestRecruiter" rr
                            JOIN "AgencyPersonnel" ap ON ap."Id" = rr."RecruiterId"
                            WHERE ap."UserId" IN ({InvalidUsers}))
                    THEN
