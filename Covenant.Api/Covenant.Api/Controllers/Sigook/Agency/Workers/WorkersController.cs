    ...
    IAgencyService agencyService) : Controller
{
    ...
    public async Task<IActionResult> Get([FromQuery] GetWorkerProfileFilter filter) =>
        Ok(await workerRepository.GetWorkersProfile(User.GetAgencyId(), filter));
    ...
    {
        var data = workerRepository.GetAllWorkersProfile(User.GetAgencyId(), filter).ToList();
    ...
    {
        var detail = await workerRepository.GetWorkerProfileDetail(wp => wp.Id == id);
    ...
    public async Task<IActionResult> OtherDocument(Guid id) =>
        Ok(await workerRepository.GetOtherDocuments(id));
    ...
    {
        var entity = await workerRepository.GetProfile(w => w.Id == id);
    ...
    {
        var data = await workerRepository.GetWorkerProfilesDropdown(User.GetAgencyIds(), searchTerm);
        return Ok(data);
    }
}
