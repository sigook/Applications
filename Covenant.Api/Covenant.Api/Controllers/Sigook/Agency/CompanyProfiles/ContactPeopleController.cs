...
public class ContactPeopleController(ICompanyRepository companyRepository, IAgencyService agencyService) : Controller
{
    public const string RouteName = "api/agency/companyprofiles/{profileId}/ContactPeople";
    ...
    public async Task<IActionResult> Post(Guid profileId, [FromBody] CompanyProfileContactPersonModel model)
    {
        ...
        return CreatedAtAction(nameof(GetById), new { profileId, id = result.Value }, new CompanyProfileContactPersonModel { Id = result.Value });
    }

    /// <summary>Gets the contact people of the specified company profile.</summary>
    /// <param name="profileId">Identifier of the company profile.</param>
    [HttpGet]
    [ProducesResponseType(typeof(IEnumerable<CompanyProfileContactPersonModel>), StatusCodes.Status200OK)]
    public async Task<IActionResult> Get(Guid profileId) => Ok(await companyRepository.GetContactPeople(c => c.CompanyProfileId == profileId));
    ...
    {
        var model = await companyRepository.GetContactPersonDetail(profileId, id);
