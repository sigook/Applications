...
abstract class WorkerProfileModel with _$WorkerProfileModel {
  ...
  const factory WorkerProfileModel({
    ...
    @Default([]) List<ProfileImageModel> otherDocuments,
  ...
  WorkerProfile toEntity() {
    return WorkerProfile(
      ...
      identificationType1Id: identificationType1?.id,
      identificationType2Id: identificationType2?.id,
      ...
      provinceId: location?.city?.province?.id,
      country: location?.city?.province?.country?.value,
      countryCode: location?.city?.province?.country?.code,
      ...
      locationPreferences: locationPreferences.map((e) => e.value ?? '').where((v) => v.isNotEmpty).toList(),
      locationPreferenceIds: locationPreferences.map((e) => e.id ?? '').where((v) => v.isNotEmpty).toList(),
