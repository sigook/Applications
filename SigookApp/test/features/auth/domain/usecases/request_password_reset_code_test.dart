...
void main() {
  ...
  late RequestPasswordResetCode usecase;

  const tEmail = 'test@example.com';

  setUp(() {
    mockRepo = MockAuthRepository();
    usecase = RequestPasswordResetCode(mockRepo);
  });

  test('delegates the email to the repository and returns success', () async {
    when(() => mockRepo.requestPasswordResetCode(tEmail))
        .thenAnswer((_) async => const Right(null));

    final result = await usecase(
      RequestPasswordResetCodeParams(email: tEmail),
    );

    expect(result.isRight(), true);
    verify(() => mockRepo.requestPasswordResetCode(tEmail)).called(1);
  });
