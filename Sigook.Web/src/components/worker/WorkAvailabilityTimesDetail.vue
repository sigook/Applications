<template>
  <div class="hover-transform">
    <div class="detail-worker-profile">
      <span class="width-30">{{ $t("WorkerTime") }}</span>
      <span class="width-70 items">
        <b-taglist>
          <b-tag v-for="item in worker.availabilityTimes" :key="item.value" type="is-info is-light" size="is-medium" rounded>{{ item.value }}</b-tag>
        </b-taglist>
      </span>
    ...
    <b-modal v-model="modalAvailability" width="500px">
      <availability-times-edit :data="worker" @closeModal="() => closeModalEdit()" />
    </b-modal>
  </div>
</template>
<script lang="ts">
export default {
  ...
  components: {
    availabilityTimesEdit: () => import("./WorkAvailabilityTimesForm.vue")
  }
}
</script>