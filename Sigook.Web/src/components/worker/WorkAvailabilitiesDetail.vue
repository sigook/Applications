  ...
  <div class="hover-transform">
    <div class="detail-worker-profile">
      ...
      <span class="width-70 items">
        <b-taglist>
          <b-tag v-for="item in props.worker.availabilities" :key="item.value" type="is-info is-light" size="is-medium" rounded>{{ item.value }}</b-tag>
        </b-taglist>
      </span>
      <b-button type="is-info" outlined rounded icon-right="pencil" @click="modalAvailability = true"></b-button>
    </div>
     <b-modal custom-content-class="card" v-model="modalAvailability" width="500px">
      <availability-edit :data="props.worker" @closeModal="() => closeModalEdit()" />
    </b-modal>
  </div>
</template>
<script setup lang="ts">
import { ref } from 'vue';
import AvailabilityEdit from './WorkAvailabilitiesForm.vue';
